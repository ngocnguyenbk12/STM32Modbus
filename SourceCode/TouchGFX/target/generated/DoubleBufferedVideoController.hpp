...
public:
    ...
    bool updateFrame(const Handle handle, touchgfx::VideoWidget& widget)
    {
        ...
        if (stream.nextBuffer)
        {
            ...
            // Signal decoder thread to wake up, because we have a free buffer
    ...
    void endFrame()
    {
        ...
        for (uint32_t i = 0; i<no_streams; i++)
        {
            ...
            {
                stream.doDecodeNewFrame = decodeForNextTick(stream);
                post |= stream.doDecodeNewFrame;
            }
        }
        MUTEX_UNLOCK(mutexBuffers);

        // Signal decoder thread to wake up and start decoding
        if (post)
        {
            SEM_POST(semDecode);
        }
